...
export const symbols: FaceButtonStyle = {
	...
	down: {
		Icon() {
			...
			svgPath.moveTo(path, scale(-crossStrokeLength / 2, antiDiagonalUnit), {absolute: true});
			svgPath.lineTo(path, scale(crossStrokeLength, antiDiagonalUnit));
			svgPath.moveTo(path, scale(-crossStrokeLength / 2, diagonalUnit), {absolute: true});
			svgPath.lineTo(path, scale(crossStrokeLength, diagonalUnit));

			return <path d={path.toString()} stroke-width={symbolsStrokeWidth} class={css.maskInStroke} />;
		},
		name: 'cross',
	},
	right: {
