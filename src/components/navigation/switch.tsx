...
export const NavSwitch: FunctionComponent<SwitchProps> = props => {
	...
	return <Switch>
		{props.children.map(vnode => cloneElement(vnode, {
			children: (parameters: any) => <navigation.Provider value={node.newChildToken()}>{typeof vnode.props.children === 'function' ? vnode.props.children(parameters) : vnode.props.children}</navigation.Provider>,
		}))}
	</Switch>;
};
